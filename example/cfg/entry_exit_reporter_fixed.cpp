 ...
 public:
  ...
  auto on(ut::events::assertion_fail<TExpr> assertion) -> void {
    std::cout << indent() << assertion.location.file_name() << ':' 
              << assertion.location.line() << ": " << colors_.fail 
              << "Failure" << colors_.none << '\n';
    std::cout << indent() << "  Condition: " << assertion.expr << '\n';
    ++assertion_count_;
    ++assertion_fails_;
  }

  auto on(ut::events::exception const& exception) -> void {
...
// Override the global runner to use our reporter
template <>
auto ut::cfg<ut::override> = ut::runner<cfg::entry_exit_reporter>{};

// Example usage - tests will automatically use the entry/exit reporter
int main() {
  using namespace ut;
  ...
  "failing test"_test = [] {
    expect(1 == 2_i) << "This should fail";
  };
  
  skip / "skipped test"_test = [] {
    expect(false) << "This should not run";
  };
